    ...
    if (isset($_POST['save'])) {
        ...
        for ($i=0;$i<count($st_id);$i++) {
          ...
          if (! empty($st_quantite_add[$i])){
                ...
                $total_add_magasinn = ((int)$st_quantite[$i]- (int)$st_quantite_add[$i]);
                $sql = "UPDATE tsb_stocks SET st_quantite = ? WHERE pr_id_fk = ?";
                $bdd->prepare($sql)->execute([$total_add_magasinn, $pr_id_fk[$i]]);
                

                $total_add_magasin = ((int)$stc_quantite[$i]+ (int)$st_quantite_add[$i]);
                ...
                header("Location: ../pages/bar_etat_stock_comptoir.php");
