<?php 
    ...
    if (isset($_POST['save_close_facture'])) {
        ...
        //$fa_status = 
        $pr_designation = $_POST['pr_designation_close'];

        for ($i=0;$i<count($pr_designation);$i++) {
            //if($st_quantite_add[$i] != 0) {
                $fa_code[$i] = $_POST['fa_code_close'];
                $fa_status_close[$i] = $_POST['fa_status_close'];
    ...
    if (isset($_POST['save_add_facture'])) {
        ...
        for ($i=0;$i<count($pr_designation);$i++) {
            ...
            $req = $bdd->query("SELECT pr_designation FROM tsb_factures WHERE pr_designation = '$pr_designation'");
